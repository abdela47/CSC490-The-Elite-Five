...
The model was later optimized to feature varying numbers of filters at each convolutional layer, so that earlier layers can extract general patterns while later layers can extract finer details. Batch normalization was also added after each layer to improve model stability. An early stopping mechanism was also implemented so prevent overfitting.
...
Another improvement would be to use a prebuilt model for image classification, such as AlexNet. Using a prebuilt model would have allowed us to spend more time on optimizations, which would have helped us achieve the 60-65% accuracy that other projects were able to achieve.
